...
import { ArcadeCollider, ArcadeImageProps, useGameLoop, useScene, ArcadeImage } from 'react-phaser-fiber';

export interface BallProps extends Omit<ArcadeImageProps, 'ref' | 'texture' | 'x' | 'y'> {
    paddleRef: React.RefObject<Phaser.Physics.Arcade.Image>;
    snapToPaddle: boolean;
    onReset: () => void;
}

function Ball({ paddleRef, snapToPaddle, onReset, ...props }: BallProps, ref: React.Ref<Phaser.Physics.Arcade.Image>) {
    const scene = useScene();
    const ball = useMemo(() => new Phaser.Physics.Arcade.Image(scene, 0, 0, 'assets', 'ball1'), [scene]);
    ...
    return (
        <ArcadeImage instance={ball} name="ball" bounce={1} collideWorldBounds {...props}>
            <ArcadeCollider
                with="paddle"

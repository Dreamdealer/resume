        ...
        useCallback(() => {
            ...
            if (state.blocks.length === 0) {
                if (paddleRef.current && ballRef.current) {
                    ...
                    ballRef.current.setPosition(paddleRef.current.x, paddleRef.current.y - 48);
...
function reducer(state: BreakoutState, action: { type: string; payload?: any }): BreakoutState {
    switch (action.type) {
        case 'RESET_GAME': {
            return defaultState;
        }
